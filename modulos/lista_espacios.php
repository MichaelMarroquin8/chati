...
$query = "SELECT * FROM espacios WHERE estado='disponible' AND sitiore='$sitiotw' AND nomb='$nombe'
";
...
if($total_data > 0)
{
  foreach($result as $row)
  {
	...
	$desbloc='desbloqueado';
	 
	
	
	$nombespac=$row['nombespac'];
	$ref=$row['idcliente'];
	$tokenc=$row['tokenc'];
...
<td><a href="canal/home.php?tokenc=<?php echo $tokenc;?>&ref=<?php echo $ref;?>&nombespac=<?php echo $nombespac;?>" target="_blank"><img src="imagenes/bor.svg" width='30px'></a></td>
<?php
if ($estatusad=='Administrador'){
?>
<td><?php echo "<a href='eliminar/elim_espacios.php?idesp=".$row['idesp']."&estado=".$nuevoestatus."&quieneli=".$usur."' id='txt_test' onclick='return ConfirmDelete()'><img src='imagenes/basura.svg' width='30px'></a>";?></td>
<td>
<?php
if ($bloqueado=='bloqueado'){
?>
<?php echo "<a href='eliminar/bloc_canal.php?idesp=".$row['tokenc']."&desbloc=".$desbloc."&quienbloc=".$usur."' id='txt_test' onclick='return ConfirmDeletedesbloc()' style='background:#31ee81;color:#ffffff;border-radius:10px;padding:5px;' >Desbloquear</a>";?>
<?php	
}else{
?>
<?php echo "<a href='eliminar/bloc_canal.php?idesp=".$row['tokenc']."&bloc=".$bloc."&quienbloc=".$usur."' id='txt_test' onclick='return ConfirmDeletebloc()' style='background:red;color:#ffffff;border-radius:10px;padding:5px;' >Bloquear</a>";?>
<?php	
}
?>
</td>
